        ...
        with tf.contrib.slim.arg_scope(model_arg_scope(batch_norm_decay=batch_norm_decay)):
            with arg_scope([layers.batch_norm], is_training=is_training):
                ...
                with tf.variable_scope("image_level_features"):
                    inputs_size = tf.shape(inputs)[2:4]
                    # global average pooling
                    image_level_features = tf.reduce_mean(
                        inputs, ve_pixels, name='global_average_pooling', keepdims=True)
...
def deeplab_v3_plus_generator(num_classes,
    ...
    """
    assert data_format == ve_data_format, "Only NCHW supported by Aurora!"
    ...
    if base_architecture == 'resnet_v2_50':
        ...
        raise ValueError(
            "'base_architrecture' {ba} not supported.".format(ba=base_architecture))
        ...
        if is_training:
            if pre_trained_model:
                ...
                variables_to_restore = tf.contrib.slim.get_variables_to_restore(
                    exclude=exclude)
                tf.train.init_from_checkpoint(pre_trained_model, {v.name.split(':')[
                                              0]: v for v in variables_to_restore})

        #inputs_size = tf.shape(inputs)[1:3]
        inputs_size = tf.shape(inputs)[2:4]
        net = end_points[base_architecture + '/block4']
        encoder_output = atrous_spatial_pyramid_pooling(
            net, output_stride, batch_norm_decay, is_training, model_arg_scope=model_arg_scope)
                ...
                with arg_scope([layers.batch_norm], is_training=is_training):
                    ...
                    with tf.variable_scope("upsampling_logits"):
                        if decoder == 'bilinear':
                            ...
                            encoder_channels = encoder_output.shape.as_list()[ve_axis]
                            low_level_channels = low_level_features.shape.as_list()[ve_axis]
                            inputs_channels = inputs.shape.as_list()[ve_axis]
                            net = tf.layers.conv2d_transpose(inputs=encoder_output, strides=(2, 2),
                                                             kernel_size=(3, 3), padding='same',
                                                             data_format=ve_chan_format,
                            ...
                            net = tf.concat([net, low_level_features], axis=ve_axis, name='concat')
                            net = layers_lib.conv2d(net, 256, [3, 3], stride=1, scope='conv_3x3_1', data_format=ve_data_format)
                            net = layers_lib.conv2d(net, 256, [3, 3], stride=1, scope='conv_3x3_2', data_format=ve_data_format)
                            # two 2x deconvs instead of the 4x bilinear scale
                            net = tf.layers.conv2d_transpose(inputs=net, strides=(2, 2), kernel_size=(3, 3),
                                                             padding='same', data_format=ve_chan_format, filters=256,
                                                             kernel_initializer=tfk.initializers.he_uniform(),
                                                             bias_initializer=tf.initializers.zeros(),
                                                             kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=_WEIGHT_DECAY))
                            net = tf.layers.conv2d_transpose(inputs=net, strides=(2, 2), kernel_size=(3, 3),
                            ...
                            if decoder == 'deconv1x':
                                ...
                                skip = tf.layers.conv2d(inputs, 64, [3, 3], padding='same', data_format=ve_data_format,
                                                        kernel_initializer=tfk.initializers.he_uniform(),
                                                        bias_initializer=tf.initializers.zeros(),
                                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=_WEIGHT_DECAY))
                                skip = tf.layers.conv2d(skip, 128, [3, 3], padding='same', data_format=ve_data_format,
                                                        kernel_initializer=tfk.initializers.he_uniform(),
                                                        bias_initializer=tf.initializers.zeros(),
                                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=_WEIGHT_DECAY))
                                net = tf.concat([net, skip], axis=ve_axis)
                                net = tf.layers.conv2d(net, 256, [3, 3], padding='same', data_format=ve_data_format,
                                                       kernel_initializer=tfk.initializers.he_uniform(),
                                                       bias_initializer=tf.initializers.zeros(),
                                                       kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=_WEIGHT_DECAY))
                                net = tf.layers.conv2d(net, 256, [3, 3], padding='same', data_format=ve_data_format,
